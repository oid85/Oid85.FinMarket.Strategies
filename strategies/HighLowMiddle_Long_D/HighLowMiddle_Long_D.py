...
class HighLowMiddle_Long_D(bt.Strategy):
    settings = {
        'id': 'cf264e4d-f3b8-414a-9975-3ccf1bcacbd5',
        'description': 'Пробой Highest, выход по Middle',
        'version': 1,
        'timeframe': 'D'
    }
    ...
    def next(self):
        ...
        if self.index >= self.params.period:
            middle = (self.highest[0] + self.lowest[0]) / 2.0
            signal_open_long = self.close[0] > self.highest[-1]
            signal_close_long = self.close[0] < middle
